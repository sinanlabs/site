...
if os.path.exists(os.path.join(HERE, "static")):   # 站长平台验证文件等原样放根目录
    for f in os.listdir(os.path.join(HERE, "static")):
        src_ = os.path.join(HERE, "static", f); dst_ = os.path.join(OUT, f)
...
<div class="wrap"><div class="brandband"><img src="/brand/sinanlab-lockup.png" srcset="/brand/sinanlab-lockup@2x.png 2x" width="480" height="123" alt="SinanLab · 司南实验室" loading="lazy"><span>方向清晰，判断有据。</span></div></div><footer class="ft"><div class="wrap in"><span>© 2026 Sinan Lab · 司南实验室</span><a href="/constitution">为什么可信</a><a href="/disclosure">收入透明</a><a href="https://github.com/sinanlabs" rel="noopener">GitHub</a><a href="/privacy">隐私政策</a><a href="/disclaimer">免责声明</a><a href="%s/method">方法论与数据</a><a href="%s/feed.xml">RSS</a><a href="mailto:hello@sinanlab.com">hello@sinanlab.com</a><span style="margin-left:auto">每个数字可追溯来源 · 不收任何被测渠道的钱</span></div></footer>
<script>(function(){var b=document.getElementById("auth");if(!b)return;fetch("%s/api/me",{credentials:"include"}).then(function(r){return r.json();}).then(function(m){if(m&&m.user){b.innerHTML='<a href="%s/me" title="我的关注">'+(m.user.avatar_url?'<img src="'+m.user.avatar_url+'" alt="">':'我的')+'</a>';}else if(m&&m.login){var H="%s/login";b.innerHTML='<a class="pl" href="'+H+'?return_to='+encodeURIComponent(location.href)+'">登录</a><a class="pl" href="'+H+'?mode=signup&return_to='+encodeURIComponent(location.href)+'">注册</a>';}}).catch(function(){});})();</script>%s</body></html>""" % (
...
<div class="txt"><img class="lockup" src="/brand/sinanlab-lockup.png" srcset="/brand/sinanlab-lockup@2x.png 2x" width="480" height="123" alt="SinanLab · 司南实验室"><div class="eyebrow">司南实验室 · AI 基础设施的中立测量者</div><h1>看清算力，<br>才好买算力。</h1>
...
<div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap"><a class="btn p" href="%s/login?return_to=%s/">登录 →</a><a class="btn o" href="%s/method">先看看我们怎么算的</a></div></section>
...
""" % (st["confirmed"], C, C, C, next((m["n_relay"] for m in D["models"] if m["id"] == "deepseek-v4-pro"), 0), C, R, format(st["quotes"], ","), st["confirmed"], len(robo_models), kpi_html, len(board_models), GEN, D["fx"]["rate"], board,
       ...
       C, BASE, C, feed, C, C, C)
ld_home = [{"@context": "https://schema.org", "@type": "Organization", "name": "Sinan Lab", "alternateName": "司南实验室", "url": BASE, "logo": BASE + "/brand/sinanlab-mark.png", "email": "hello@sinanlab.com", "sameAs": ["https://github.com/sinanlabs"]},
...
<p class="lead" style="margin-top:14px">想按站、按模型收提醒：登录后在站点页或模型页点"关注"。</p><a class="btn o" style="margin-top:10px" href="%s/login?return_to=%s/subscribe">登录 →</a></section>
